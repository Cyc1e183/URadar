...
################################ Mutation operation########################################
    def mutation_chain(self, base_chain, seed_type, success_list, PublicSuffixDetection_FailList):    
      ...
      ########################################## Set up control rules ##########################################
        ...
        for i in op_range:
            #print i.lower()
            IMCFOpration = i.split('_')[0].lower()
            IMCFfiletype = i.split('_')[-1].lower()
            excludedflag = True
            if (seed_type not in op_range_import[i].mOP.__seed_dependency__) or (i.split('_')[-1].lower() in PublicSuffixDetection_FailList):# or (i.split('_')[-1][-3:].lower() in PublicSuffixDetection_FailList):
                excludedflag = False
            else:
                for ele in base_chain_import.keys():
                    #print base_chain_import[ele].mOP.__exclusion_op__.keys()
                    #print ele.split('_')[-1].lower()
                    if (seed_type not in base_chain_import[ele].mOP.__exclusion_op__.keys()) or (ele.split('_')[-1].lower() in PublicSuffixDetection_FailList):# or (ele.split('_')[-1][-3:].lower() in PublicSuffixDetection_FailList):
                        excludedflag = False
                        break
                    elif (i in base_chain_import[ele].mOP.__exclusion_op__[seed_type]) or (i.split('_')[-1].lower() in PublicSuffixDetection_FailList):# or (i.split('_')[-1][-3:].lower() in PublicSuffixDetection_FailList): ##### black list
                        excludedflag = False
                        break
                    elif((IMCFOpration in IMCFList) and (ele.split('_')[0].lower() in IMCFList) and (IMCFfiletype !=ele.split('_')[-1].lower())):
                        excludedflag = False
                        break
            ##############################################################################################################################
            if excludedflag:
